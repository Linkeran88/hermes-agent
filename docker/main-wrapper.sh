#!/command/with-contenv sh
    ...
    if [ $# -eq 0 ]; then
        # Railway/cloud: if GATEWAY_ONLY is set, start the gateway directly
        # instead of the interactive CLI (which requires a terminal).
        if [ "${GATEWAY_ONLY:-}" = "1" ] || [ "${GATEWAY_ONLY:-}" = "true" ]; then
